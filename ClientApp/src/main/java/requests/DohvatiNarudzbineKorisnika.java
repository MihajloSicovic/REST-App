...
import services.NarudzbinaService;
...
public class DohvatiNarudzbineKorisnika {
    ...
    public static void request(KorisnikModel korisnik) {
        ...
        OkHttpClient client = new OkHttpClient.Builder()
            ...
            NarudzbinaService service = retrofit.create(NarudzbinaService.class);

            try {
                Call<List<NarudzbinaModel>> call = service.getNarudzbinaByIdK(idK);
                Response<List<NarudzbinaModel>> response = call.execute();
                
                System.out.println("idN, ukupnaCena, vremeKreiranja, adresa, idG, idK");
                for (NarudzbinaModel n: response.body()) {
